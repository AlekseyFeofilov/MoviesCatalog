...
struct FavoriteStack: View {
    @ObservedObject var viewModel: FavoriteStackViewModel
    ...
    var body: some View {
        ...
        GeometryReader { outerProxy in
            ScrollView(.horizontal, showsIndicators: false){
                HStack(alignment: .top, spacing: mediumPadding) {
                    ForEach(viewModel.movies) { movie in
                        ZStack {
                            ...
                            FavoriteMovie(url: movie.poster ?? "", deletAction: viewModel.deleteFavoriteMovie(movie))
                                .frame(maxWidth: movie.id == viewModel.firstVisibleMovie ? firstFavoriteMovieWidth : favoriteMovieWidth, maxHeight: movie.id == viewModel.firstVisibleMovie ? firstFavoriteMovieHeight : favoriteMovieHeight)
                                .animation(.easeIn(duration: favoriteMovieAnimationDuration), value: viewModel.firstVisibleMovie)
                                .onTapGesture {
